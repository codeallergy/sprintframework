...
docker-target:
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o $(TARGET)/$(BIN)_linux -v -ldflags "-X main.Version=$(VERSION) -X main.Build=$(NOW)"
	docker run -it --rm -v /Users/ashvid/go/src/github.com/codeallergy/sprintframework/target:/target ubuntu:16.04
...
licenses:
	go-licenses csv "github.com/codeallergy/sprintframework" > resources/licenses.txt
